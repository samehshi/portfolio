...
class AlphaVantageAPI:
    ...
    def get_daily(self, ticker, output_size='full'):
        ...
        # This line is added by me sameh shehata
        ...
        if 'Time Series (Daily)' not in response_data.keys():
            raise Exception(
                f"Invalid API Call. Check that ticker sympole {ticker} is correct."
            )
...
class SQLRepository:
    ...
    def read_table(self,table_name , limit=None):
        ...
        df_new =  pd.read_sql(sql = query , con = self.connection , index_col="date" , parse_dates='date')
