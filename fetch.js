...
function validateEnvironment() {
  ...
  if (USE_MEDIUM_DATA === "true") {
    if (!MEDIUM_USERNAME) {
      warnings.push(
        "MEDIUM_USERNAME not set, Medium integration will be disabled"
      );
    }
  }
      ...
      const req = https.request(requestOptions, res => {
        ...
        if (res.statusCode !== 200 && res.statusCode !== 422) {
          ...
          if (attempt < maxRetries) {
            ...
            console.error(
              `❌ Max retries (${maxRetries}) reached. Request failed.`
            );
            res.resume();
            reject(error);
            return;
...
async function fetchAllData() {
  try {
    ...
    if (env.USE_GITHUB_DATA === "true") {
      ...
      console.warn(
        "⚠️  USE_GITHUB_DATA is set to false. Skipping GitHub data fetching."
      );
    }
    ...
    if (env.USE_MEDIUM_DATA === "true" && env.MEDIUM_USERNAME) {
      ...
      console.warn(
        "⚠️  USE_MEDIUM_DATA is set to false or MEDIUM_USERNAME is not defined. Skipping Medium blog data fetching."
      );
    }
